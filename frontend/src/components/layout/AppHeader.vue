...
const breadcrumbs = computed(() => {
  ...
  // Find parent from route meta
  if (route.meta?.parent) {
    const parentRoute = router.getRoutes().find((r) => r.name === route.meta.parent)
    if (parentRoute) {
      crumbs.push({
        label: parentRoute.meta?.title || parentRoute.name,
        to: parentRoute.path
      })
    }
  }
  ...
  >
    ...
    <nav class="flex items-center gap-1.5 text-sm">
      <template v-if="breadcrumbs.length > 0">
        ...
        <template v-for="(crumb, i) in breadcrumbs" :key="i">
          ...
          <span v-else class="font-medium text-go4-secondary dark:text-gray-100">
            {{ crumb.label }}
          </span>
        </template>
      </template>
    ...
    <div class="flex items-center gap-1">
      ...
      <div class="relative">
        ...
        <div v-if="userMenuOpen" class="fixed inset-0 z-[-1]" @click="closeMenuOnOutsideClick" />
      </div>
    </div>
  </header>
