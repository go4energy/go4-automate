...
<template>
  ...
  >
      ...
      <div class="flex items-start gap-4">
        ...
        <div class="flex-1">
          ...
          <div class="mt-1 flex flex-wrap gap-3 text-xs text-gray-600 dark:text-gray-400">
            ...
            >Premium</span>
            <span
              v-if="profile.connection_status"
              class="font-medium text-blue-600"
            >{{ profile.connection_status }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <h4 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
        Berufserfahrung ({{ profile.experience.length }})
      </h4>
      <div
        v-for="(exp, i) in profile.experience"
        :key="i"
        class="mb-3 border-b border-gray-100 pb-3 last:mb-0 last:border-0 last:pb-0 dark:border-gray-700"
      >
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          <a
            v-if="exp.company_linkedin_url"
            :href="exp.company_linkedin_url"
            target="_blank"
            class="hover:text-[#0A66C2] hover:underline"
          >{{ exp.company }}</a>
          <span v-else>{{ exp.company }}</span>
          <span
            v-if="exp.employment_type"
            class="text-gray-400"
          > · {{ exp.employment_type }}</span>
        </p>
        <p
          v-if="exp.date_range"
    ...
    >
      <h4 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
        Ausbildung ({{ profile.education.length }})
      </h4>
      <div
      ...
      >
        ...
        <p
          v-if="edu.start_year || edu.end_year"
          class="text-xs text-gray-500"
        >
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        <span
          ...
          class="rounded-full bg-blue-100 px-3 py-1 text-xs text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      ...
      >
        ...
        <div class="space-y-1 text-sm text-gray-600 dark:text-gray-400">
          <p v-if="profile.contact_info.email">
            Email: {{ profile.contact_info.email }}
          </p>
          <p v-if="profile.contact_info.phone">
            Telefon: {{ profile.contact_info.phone }}
          </p>
          <p v-if="profile.contact_info.website">
            Web: {{ profile.contact_info.website }}
          </p>
          <p v-if="profile.contact_info.address">
            Adresse: {{ profile.contact_info.address }}
          </p>
          <p v-if="profile.contact_info.birthday">
            Geburtstag: {{ profile.contact_info.birthday }}
          </p>
          <p v-if="profile.contact_info.twitter_url">
            Twitter: {{ profile.contact_info.twitter_url }}
          </p>
          <p v-if="profile.contact_info.connected_since">
            Vernetzt seit: {{ profile.contact_info.connected_since }}
          </p>
        </div>
      </div>
    </div>
    ...
    <!-- Interests -->
    <div
      v-if="profile.interests && Object.values(profile.interests).some(v => v?.length)"
      class="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800"
    >
      <h4 class="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
        Interessen
      ...
      <div class="grid grid-cols-2 gap-3 md:grid-cols-5">
        <div
          v-for="(items, cat) in profile.interests"
          :key="cat"
        >
          <p
            v-if="items?.length"
    ...
    >
      ...
      <div class="flex flex-wrap gap-2">
        <span
          ...
          :key="tag"
          class="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-700 dark:bg-gray-700 dark:text-gray-300"
        >#{{ tag }}</span>
      </div>
    </div>
