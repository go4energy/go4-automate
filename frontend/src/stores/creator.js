...
export const useCreatorStore = defineStore('creator', () => {
  ...
  async function fetchPieces(params = {}) {
    ...
    error.value = null
    try {
      const { data } = await getCreatorPieces(params)
      pieces.value = data
    } catch (err) {
  ...
  async function fetchPiece(pieceId) {
    ...
    error.value = null
    try {
      const { data } = await getCreatorPiece(pieceId)
  ...
  async function update(pieceId, data) {
    error.value = null
    try {
      const { data: piece } = await updateCreatorPiece(pieceId, data)
      const index = pieces.value.findIndex((p) => p.id === pieceId)
      if (index !== -1) pieces.value[index] = piece
      if (currentPiece.value?.id === pieceId) currentPiece.value = piece
      return piece
  ...
  async function remove(pieceId) {
    error.value = null
    try {
      await deleteCreatorPiece(pieceId)
