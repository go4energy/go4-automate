...
async def test_create_content_piece(client, test_tenant):
    """POST /api/v1/creator/pieces should create a content piece."""
    response = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
...
async def test_list_content_pieces(client, test_tenant):
    """GET /api/v1/creator/pieces should list pieces for tenant."""
    await client.post("/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS)
    second = {**PIECE_PAYLOAD, "title": "Zweiter Post", "platform": "instagram"}
    await client.post("/api/v1/creator/pieces", json=second, headers=HEADERS)

    response = await client.get("/api/v1/creator/pieces", headers=HEADERS)
...
async def test_list_content_pieces_filter_status(client, test_tenant):
    """GET /api/v1/creator/pieces?status=draft should filter by status."""
    await client.post("/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS)

    response = await client.get("/api/v1/creator/pieces?status=draft", headers=HEADERS)
    assert response.status_code == 200
    data = response.json()
...
async def test_list_content_pieces_filter_platform(client, test_tenant):
    """GET /api/v1/creator/pieces?platform=linkedin should filter by platform."""
    await client.post("/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS)
    other = {**PIECE_PAYLOAD, "title": "Insta Post", "platform": "instagram"}
    await client.post("/api/v1/creator/pieces", json=other, headers=HEADERS)

    response = await client.get(
        "/api/v1/creator/pieces?platform=linkedin", headers=HEADERS
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
async def test_get_content_piece(client, test_tenant):
    """GET /api/v1/creator/pieces/{id} should return a single piece."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]

    response = await client.get(f"/api/v1/creator/pieces/{piece_id}", headers=HEADERS)
    assert response.status_code == 200
...
async def test_get_nonexistent_piece(client, test_tenant):
    """GET /api/v1/creator/pieces/999 should return 404."""
    response = await client.get("/api/v1/creator/pieces/999", headers=HEADERS)
    assert response.status_code == 404


@pytest.mark.anyio
async def test_update_content_piece(client, test_tenant):
    """PUT /api/v1/creator/pieces/{id} should update a piece."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]
    ...
    response = await client.put(
        f"/api/v1/creator/pieces/{piece_id}",
...
async def test_delete_content_piece(client, test_tenant):
    """DELETE /api/v1/creator/pieces/{id} should delete a piece."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]
    ...
    response = await client.delete(
        f"/api/v1/creator/pieces/{piece_id}", headers=HEADERS
    )
    assert response.status_code == 204

    get_resp = await client.get(f"/api/v1/creator/pieces/{piece_id}", headers=HEADERS)
    assert get_resp.status_code == 404


@pytest.mark.anyio
async def test_create_calendar_entry(client, test_tenant):
    """POST /api/v1/creator/calendar should create a calendar entry."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]
    ...
    response = await client.post(
        "/api/v1/creator/calendar", json=cal_payload, headers=HEADERS
    )
    assert response.status_code == 201
    data = response.json()
...
async def test_list_calendar(client, test_tenant):
    """GET /api/v1/creator/calendar should list calendar entries."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]
    ...
    await client.post("/api/v1/creator/calendar", json=cal_payload, headers=HEADERS)

    response = await client.get("/api/v1/creator/calendar", headers=HEADERS)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
async def test_generate_content(client, test_tenant):
    """POST /api/v1/creator/pieces/generate should create an AI-generated draft."""
    mock_json = {
    ...
    with patch(
        "app.creator.service.LLMService.generate_json",
        new_callable=AsyncMock,
        return_value=mock_json,
    ):
        response = await client.post(
            "/api/v1/creator/pieces/generate",
            json={
...
async def test_approve_content(client, test_tenant):
    """PATCH /api/v1/creator/pieces/{id}/approve should set scheduled status."""
    create_resp = await client.post(
        "/api/v1/creator/pieces", json=PIECE_PAYLOAD, headers=HEADERS
    )
    piece_id = create_resp.json()["id"]
    ...
    response = await client.patch(
        f"/api/v1/creator/pieces/{piece_id}/approve",
        json={
            "approved_by": "tester",
...
async def test_publish_requires_scheduled(client, test_tenant):
    ...
    response = await client.post(
        f"/api/v1/creator/pieces/{piece_id}/publish",
        headers=HEADERS,
    )
    assert response.status_code == 400
...
async def test_theme_rotation(client, test_tenant):
    """POST /api/v1/creator/themes/rotation should return first topic when no history."""
    topics = ["Energie", "Solar", "Wärme"]
    response = await client.post(
        "/api/v1/creator/themes/rotation",
        json=topics,
        headers=HEADERS,
    )
    assert response.status_code == 200
