...
class CampaignPerformance(TimestampMixin, Base):
    ...
    # Relationships
    tenant = relationship("Tenant", back_populates="campaign_performances")
    campaign = relationship("Campaign", back_populates="performances")

    __table_args__ = (
        Index("ix_campaign_performance_tenant_date", "tenant_id", "date"),
    )

    def __repr__(self) -> str:
        return (
            f"<CampaignPerformance {self.platform} "
            f"{self.campaign_id} ({self.date})>"
        )


class CampaignConfig(TimestampMixin, Base):
    ...
    def __repr__(self) -> str:
        return f"<CampaignConfig {self.campaign_id} ({self.status})>"
...
class CampaignConversion(Base):
    ...
    __table_args__ = (
        Index("ix_campaign_conv_tenant_event", "tenant_id", "event_name"),
        Index("ix_campaign_conv_time", "event_time"),
    )

    def __repr__(self) -> str:
        return f"<CampaignConversion {self.event_name} ({self.event_time})>"
