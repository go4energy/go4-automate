"""Ad management service - business logic for ads pipeline."""
...
from app.ads.meta_ads_client import get_campaign_insights, get_campaigns
from app.ads.models import AdCampaignConfig, ConversionEvent
from app.ads.optimizer import optimize_campaign
...
from app.ads.weather_service import get_current_weather
...
class AdService:
    ...
    async def track_conversion(
        ...
        if settings.meta_pixel_id and settings.meta_system_user_token:
            try:
                from app.ads.meta_ads_client import send_event
    ...
    async def _get_period_stats(
        ...
        result = await self.db.execute(
            select(
                ...
                func.coalesce(func.sum(AdPerformance.leads), 0),
                func.coalesce(func.sum(AdPerformance.impressions), 0),
            ).where(
                AdPerformance.tenant_id == tenant_id,
                AdPerformance.date >= from_date,
                AdPerformance.date <= to_date,
            )
        )
        row = result.one()
    ...
    async def _get_campaign_summaries(
        ...
        result = await self.db.execute(
            select(
                ...
                func.max(AdPerformance.campaign_name),
                func.coalesce(func.sum(AdPerformance.spend), 0),
                func.coalesce(func.sum(AdPerformance.leads), 0),
    ...
    async def get_performance_history(
        ...
        if campaign_id:
            query = query.where(AdPerformance.campaign_id == campaign_id)
    ...
    async def configure_campaign(
        ...
        existing = await self.db.execute(
            select(AdCampaignConfig).where(
                AdCampaignConfig.tenant_id == tenant_id,
                AdCampaignConfig.campaign_id == data.campaign_id,
            )
        )
    ...
    async def list_campaign_configs(self, tenant_id: str) -> list[AdCampaignConfig]:
        """List all campaign configs for a tenant."""
        result = await self.db.execute(
            select(AdCampaignConfig)
            .where(AdCampaignConfig.tenant_id == tenant_id)
            .order_by(AdCampaignConfig.created_at.desc())
        )
        return list(result.scalars().all())

    async def _get_config(self, tenant_id: str, config_id: int) -> AdCampaignConfig:
        """Get a campaign config, raising NotFoundError if missing."""
        result = await self.db.execute(
            select(AdCampaignConfig).where(
                AdCampaignConfig.id == config_id,
                AdCampaignConfig.tenant_id == tenant_id,
            )
        )
        config = result.scalar_one_or_none()
        if not config:
            raise NotFoundError("AdCampaignConfig", config_id)
        return config
    ...
    async def run_optimization(self, tenant_id: str) -> list[OptimizationResult]:
        ...
        configs = await self.db.execute(
            select(AdCampaignConfig).where(
                AdCampaignConfig.tenant_id == tenant_id,
                AdCampaignConfig.status == "active",
                AdCampaignConfig.auto_optimize.is_(True),
            )
        )
    ...
    async def get_weather(self) -> dict:
        ...
        from app.ads.optimizer import DEFAULT_LAT, DEFAULT_LON
